  ...
  if (dsc.statusChanged) { // Checks if the security system status has changed
    ...
    for (byte partition = 0; partition < dscPartitions; partition++) {
      ...
      if (dsc.alarmChanged[partition]) {
        ...
        if (dsc.alarm[partition]) mqttClient.publish(publishTopic, "triggered", true);  // Alarm tripped
        else if (!dsc.armedChanged[partition]) mqttClient.publish(publishTopic, "disarmed", true);
      }
