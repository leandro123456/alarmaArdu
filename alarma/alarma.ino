...
void loop() {
  ...
  if (needMqttConnect){
    ...
    if (atoi(enableMqttDebugValue) == 1) {
      mqttClient.publish(MqttDebugTopicValue,deviceID + " - MQTT reconnect", (bool) atoi(mqttRetainValue),atoi(mqttQoSValue));
    }
    connectMqtt();
  }
  ...
  if (needReset){
    ...
    if (atoi(enableMqttDebugValue) == 1) {
      mqttClient.publish(MqttDebugTopicValue, deviceID + " - Rebooting after 1 second..." , (bool) atoi(mqttRetainValue), atoi(mqttQoSValue))
    ;}
  ...
  if (iotWebConf.getState() == iotwebconf::OnLine) {
    if ( atoi(updateIntervalValue) > 0 && millis() - lastPublish > (atoi(updateIntervalValue) * 1000) ) { 
      ...
      if (atoi(enableMqttDebugValue) == 1) {
        mqttClient.publish(MqttDebugTopicValue, deviceID + " - ["+"] - Keep Alive interval elapsed. Automatically publishing...", (bool) atoi(mqttRetainValue), atoi(mqttQoSValue));
      }
      publicaEstados();
      lastPublish = millis();
    }
...
void configSaved(){
  ...
  if (atoi(enableMqttDebugValue) == 1) {
    mqttClient.publish(MqttDebugTopicValue, deviceID + " - Configuration updated.", (bool) atoi(mqttRetainValue), atoi(mqttQoSValue));
  }
  needReset = true;
}
